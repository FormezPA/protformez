...
public class CreateSignature extends StreamContent {
	
	private static Logger logger = Logger.getLogger(CreateSignature.class);
	...
	public void execute(WebScriptRequest req, WebScriptResponse res) throws IOException {
		...
		logger.debug("Genarated protocol signature for protocol: " + protocolNum );
		
		res.setContentType(MimetypeMap.MIMETYPE_PDF);
		res.setHeader("Content-Disposition", "attachment; filename=\""+protocolNum+".pdf\"");
		DocumentService.signature(nodeRef, res.getOutputStream());
	}
